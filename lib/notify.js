...
class Notifier {
    ...
    _renderHtml(event, label) {
        ...
        const link = base && board.id && card.id
            ? `<p><a href="${base}/?board=${encodeURIComponent(board.id)}&amp;card=${encodeURIComponent(card.id)}" style="color:${accent}">${serverT(lang, 'mail.openCard')}</a></p>`
            : '';
